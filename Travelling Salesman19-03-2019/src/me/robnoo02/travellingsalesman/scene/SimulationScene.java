...
public class SimulationScene {
	...
	public SimulationScene(LinkedList<CityPoint> points, int size, Stage stage, boolean lines) {
			...
			public void handle(ActionEvent arg0) {
				...
				stage.setScene(
						(new SimulationScene(SimulationGenerator.randomCityPoints(Integer.valueOf(number.getText())),
								size, stage, Boolean.valueOf(linesB.getText()))).getScene());
			}

		});
		group.getChildren().addAll(number, linesB, button);
	...
	public void drawShapes() {
		...
		for(CityPoint point: points) {
			DebugState.DISTANCE_TO_ALL.debug(String.valueOf(point.getID()) + ": " + point.getDistToOthers() + " - " + String.valueOf(point.getConnectedCount()));
		}
