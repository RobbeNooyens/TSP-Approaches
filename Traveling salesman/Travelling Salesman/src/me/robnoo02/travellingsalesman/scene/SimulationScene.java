...
public class SimulationScene {
	...
	public SimulationScene(LinkedList<Point> points, int size, Stage stage, boolean lines) {
		...
		button.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				stage.setScene(
						(new SimulationScene(SimulationGenerator.randomPoints(points.size()), size, stage, lines))
								.getScene());
			}
	...
	public boolean hasIntersection(Point point11, Point point12, Point point21, Point point22) {
		...
		return point11.getX() < point12.getX() ? (point11.getX() < x && x < point12.getY())
				: (point11.getX() > x && x > point12.getY());
	}
